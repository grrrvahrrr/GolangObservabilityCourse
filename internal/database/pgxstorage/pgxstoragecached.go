...
type PgxStorageChached struct {
	db    *pgxpool.Pool
	cache *cache.Cache
}

func NewPgxStorageChached(ctx context.Context, cfg *pgxpool.Config) (*PgxStorageChached, error) {
...
func (pgxs *PgxStorageChached) WriteURL(ctx context.Context, url entities.UrlData) (*entities.UrlData, error) {
	dbd := &PgxData{
		FullURL:  url.FullURL,
...
func (pgxs *PgxStorageChached) WriteData(ctx context.Context, url entities.UrlData) (*entities.UrlData, error) {
...
func (pgxs *PgxStorageChached) ReadURL(ctx context.Context, url entities.UrlData) (*entities.UrlData, error) {
	dbd := &PgxData{}

	if url.AdminURL != "" {
		dbd.AdminURL = url.AdminURL

		key := fmt.Sprintf("adminurl:%s", dbd.AdminURL)
		err := pgxs.cache.Get(ctx, key, &dbd.ShortURL)
		if err == cache.ErrCacheMiss {
			err = pgxs.db.QueryRow(ctx, `SELECT short_url FROM bitme.adminurl WHERE admin_url = $1`, dbd.AdminURL).Scan(&dbd.ShortURL)
			if err != nil && err != pgx.ErrNoRows {
				return nil, err
			}
			err = pgxs.cache.Set(&cache.Item{
				Ctx:   ctx,
				Key:   key,
				Value: dbd.ShortURL,
				TTL:   time.Hour,
	...
	key := fmt.Sprintf("shorturl:%s", dbd.ShortURL)
	err := pgxs.cache.Get(ctx, key, &dbd.FullURL)
	if err == cache.ErrCacheMiss {
...
func (pgxs *PgxStorageChached) GetIPData(ctx context.Context, url entities.UrlData) (string, error) {
	var ipdata string
...
func (pgx *PgxStorageChached) Close() {
	pgx.db.Close()
}
